 ...
 *
 */
qx.Class.define("multiple_glwidgets.Application",
        {
    ...
    members :
    {
      ...
      -------------------------------------------------------------------------
            ...
            // create second scene
            this.GLWidget2 = new qxthree.GLWidget([], {alpha: true});
            this.GLWidget2.addListener("scriptLoaded", this.initMeshes2, this);
            this.GLWidget2.addListener("sceneCreated", this.scenePostProcess2, this);
        ...
        scenePostProcess2: function()
        {
        	//this.GLWidget2.getScene().background = new THREE.Color("ff000");
        	var renderer = this.GLWidget2.getRenderer();
        	renderer.setClearAlpha(0.0);
        	this.GLWidget2.showAxis(5);
        	
        	this.GLWidget2.setPostAnimatedMethod(function(){
        		var camera1 = this.GLWidget.getCamera();
        		var camera2 = this.GLWidget2.getCamera();
        		camera2.up = camera1.up;
        		camera2.position.copy(camera1.position);
        		camera2.position.sub(this.GLWidget.getController().target);
        		camera2.position.setLength(300);
        		camera2.lookAt(this.GLWidget2.getScene().position);
        	}.bind(this));
        },
        
        initMeshes2: function()
        {
        	var geometry = new THREE.BoxBufferGeometry( 20, 20, 20 );
        	var mat = new THREE.MeshDepthMaterial();
            var glCube = new qxthree.GLMeshModel("cube_axis", null, geometry, mat, function(){
                this.setPosition(-10, -10, -10);
                this.setScale(10, 10, 10);
            });
            this.GLWidget2.addGLModel(glCube);   
        },
        ...
        initWindow: function()
        {
            ...
            // Add the different widgets to the container. Be careful order matter!
            container.add(this.GLWidget, { width: "100%", height: "100%" });
            container.add(this.createPanel(), { left: 0 });
            container.add(this.GLWidget2, { bottom:10, left:10, width: "15%", height:"15%" });
